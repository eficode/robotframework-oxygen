...
class BaseHandler(object):
  ...
  def _report_oxygen_run(self, keyword, setup_keywords, teardown_keywords):
    ...
    teardown_keyword = self._robot_keyword(
      'Oxygen Teardown',
      teardown=True,
      keywords=self._robot_keywords(default_type=True, *teardown_keywords),
    )

    # Handle trigger keyword and report
    result_suite = self._build_results(keyword, setup_keyword, teardown_keyword)
  ...
  def _inject_suite_report(self, test, result_suite):
    ...
    test.parent.tests = [sibling for sibling in test.parent.tests if sibling is not test]
